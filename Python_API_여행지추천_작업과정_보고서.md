...
> 캐싱(caching) 관련 참고 — Python은 .py 파일을 실행할 때 재실행 속도를 높이기 위해 __pycache__/ 폴더 안에 컴파일된 .pyc 캐시 파일을 자동 생성한다. 이 파일은 프로그램 동작에 필요한 원본이 아니라 부산물이므로 삭제해도 무방하며, 다시 실행하면 자동으로 재생성된다. .gitignore에도 __pycache__/와 *.pyc를 등록해 Git 추적에서 제외했다. 다만 이 규칙이 이미 커밋된 파일에는 소급 적용되지 않아 실제로 문제가 발생한 사례가 있었는데, 자세한 내용은 14-1절과 그림 45에서 확인할 수 있다.
...
> 주의할 점 — .gitignore에 *.pyc를 등록해 두었음에도, 저장소 루트에 travel_planner.cpython-314.pyc 파일이 실제로 업로드되어 있음을 그림 45에서 확인했다(아래 14-3 참고). 이는 __pycache__ 폴더 밖에 남아 있던 캐시 파일을 함께 업로드한 실수로, 제출물 정리 시 .gitignore 규칙이 이미 추적 중인 파일에는 소급 적용되지 않는다는 점을 실무적으로 확인한 사례다. 캐시 파일(__pycache__, .pyc) 관리 방법은 프로젝트 README.md "8. 캐시 파일 관리" 항목에 별도로 정리했다.
