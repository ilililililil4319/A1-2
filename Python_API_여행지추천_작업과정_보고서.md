...
| 16. 작업 과정에서 배운 점 | 28 |
| 17. 제출 자료 최종 확인 | 29 |
| 18. 보너스 과제 제안 — 복수 지역 추천 & 결과 캐싱 | 30 |
...
> **🗂 캐싱(Caching) 관련 참고**
>
> Python은 .py 파일을 실행할 때 재실행 속도를 높이기 위해 __pycache__/ 폴더 안에 컴파일된 .pyc 캐시 파일을 자동 생성한다. 이 파일은 프로그램 동작에 필요한 원본이 아니라 부산물이므로 삭제해도 무방하며, 다시 실행하면 자동으로 재생성된다. .gitignore에도 __pycache__/와 *.pyc를 등록해 Git 추적에서 제외했다. 다만 이 규칙이 이미 커밋된 파일에는 소급 적용되지 않아 실제로 문제가 발생한 사례가 있었는데, 자세한 내용은 14-1절과 그림 45에서 확인할 수 있다.
...
> **🗂 캐싱(Caching) 관련 참고 — .gitignore의 소급 적용 한계**
>
> .gitignore에 *.pyc를 등록해 두었음에도, 저장소 루트에 travel_planner.cpython-314.pyc 파일이 실제로 업로드되어 있음을 그림 45에서 확인했다(아래 14-3 참고). 이는 __pycache__ 폴더 밖에 남아 있던 캐시 파일을 함께 업로드한 실수로, 제출물 정리 시 .gitignore 규칙이 이미 추적 중인 파일에는 소급 적용되지 않는다는 점을 실무적으로 확인한 사례다. 캐시 파일(__pycache__, .pyc) 관리 방법은 프로젝트 README.md "8. 캐시 파일 관리" 항목에 별도로 정리했다.
...
| (보너스) 복수 지역 추천, 결과 캐싱 | - | 미구현 — 설계안은 18장 참고 |
...
## 18. 보너스 과제 제안 — 복수 지역 추천 & 결과 캐싱 (미구현)

기본 요구사항(단일 지역 추천 + 실제 오류 재현 검증)을 우선 완료하는 것을 목표로 했기 때문에, 15장 점검표에 "미구현"으로 표기했던 두 보너스 기능은 이번 제출 범위에서 실제 코드로 구현하지는 않았다. 다만 두 기능이 서로 자연스럽게 이어지는 아이디어라 설계 방향을 함께 정리해 둔다.

> 여러 지역을 추천받아 비교하려면 API 호출 횟수가 늘어나는데, 결과 캐싱을 적용하면 이미 조회한 날짜·지역 조합은 재호출 없이 즉시 재사용할 수 있다. 즉 캐싱은 복수 지역 추천의 확장성을 뒷받침하는 기반 기능이 된다.

### 18-1. 복수 지역 추천 (Multiple City Recommendations)

목적 — 한 번의 실행으로 여러 후보 지역을 비교할 수 있도록 확장한다.

- LLM 프롬프트를 "도시 1곳 추천"에서 "도시 N곳을 우선순위와 함께 추천"으로 변경하고, JSON 스키마를 배열 형태({"recommendations": [{"city":..., "weather":...},...]})로 확장한다.
- 추천된 도시마다 search_restaurants()를 반복 호출해 도시별 맛집 목록을 확보한다.
- generate_report()에서 지역별 섹션(## 추천 지역 1 - 제주도, ## 추천 지역 2 - 강릉...)으로 리포트를 구성한다.
- CLI에 --count N 같은 선택적 옵션을 추가해 사용자가 추천받을 지역 수를 조절할 수 있게 설계한다.

기대 효과 — 여행지를 비교해서 고르고 싶은 사용자에게 선택지를 제공하며, 단일 추천의 "정보 부족" 한계를 보완한다.

### 18-2. 결과 캐싱 (간단한 파일 기반 캐시)

목적 — 동일한 날짜로 반복 실행할 때 불필요한 API 재호출을 막아 응답 속도를 높이고 쿼터·비용을 절약한다.

- main() 시작 시 results/{date}_data.json 파일이 이미 존재하는지 확인한다.
- 존재하면 해당 파일을 읽어 recommendation/restaurants를 그대로 재사용한다.
- --refresh 같은 옵션을 추가해, 사용자가 원할 때는 캐시를 무시하고 강제로 새로 조회할 수 있도록 예외 처리한다.
...
기대 효과 — 같은 날짜로 여러 번 테스트하거나 리포트만 다시 만들고 싶을 때 API 호출 없이 즉시 결과를 확인할 수 있다.

> **🗂 8장·10장의 __pycache__ 캐싱과의 차이**
>
> 앞서 다룬 __pycache__/.pyc는 파이썬 인터프리터가 코드를 다시 컴파일하지 않도록 돕는 실행 최적화 캐시이고, 이번 18-2의 결과 캐싱은 API 호출 결과(데이터)를 재사용하는 애플리케이션 레벨 캐시다. 성격은 다르지만 "한 번 계산·조회한 값을 저장해 두었다가 재사용한다"는 개념은 동일하다.

### 18-3. 구현 여부

두 기능 모두 설계 방향과 예시 코드만 정리했으며, 실제 travel_planner.py 코드에는 반영하지 않았다. 이후 확장한다면 위 스케치를 기반으로 get_recommendation()·search_restaurants()·main()을 수정하는 것으로 시작할 수 있다. README.md "14. 보너스 과제 제안" 항목에도 동일한 내용을 정리해 두었다.
