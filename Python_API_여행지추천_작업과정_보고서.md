...
## 18. 보너스 과제 제안 — 결과 캐싱 (미구현)

기본 요구사항(단일 지역 추천 + 실제 오류 재현 검증)을 우선 완료하는 것을 목표로 했기 때문에, 15장 점검표에 "미구현"으로 표기했던 보너스 기능은 이번 제출 범위에서 실제 코드로 구현하지는 않았다. 다만 설계 방향은 정리해 둔다.

### 18-1. 결과 캐싱 (간단한 파일 기반 캐시)
...
>
> 앞서 다룬 __pycache__/.pyc는 파이썬 인터프리터가 코드를 다시 컴파일하지 않도록 돕는 실행 최적화 캐시이고, 이번 18-1의 결과 캐싱은 API 호출 결과(데이터)를 재사용하는 애플리케이션 레벨 캐시다. 성격은 다르지만 "한 번 계산·조회한 값을 저장해 두었다가 재사용한다"는 개념은 동일하다.

### 18-2. 구현 여부

설계 방향과 예시 코드만 정리했으며, 실제 travel_planner.py 코드에는 반영하지 않았다. 이후 확장한다면 위 스케치를 기반으로 get_recommendation()·search_restaurants()·main()을 수정하는 것으로 시작할 수 있다. README.md "14. 보너스 과제 제안" 항목에도 동일한 내용을 정리해 두었다.
