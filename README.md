...
| (보너스, 미구현) 복수 지역 추천 | 설계안만 정리 — "14. 보너스 과제 제안" 참고 |
| (보너스, 미구현) 결과 캐싱(간단) | 설계안만 정리 — "14. 보너스 과제 제안" 참고 |

---

## 14. 보너스 과제 제안 — 복수 지역 추천 & 결과 캐싱 (미구현)

기본 요구사항(단일 지역 추천 + 실제 오류 재현 검증)을 우선 완료하는 것을 목표로 했기 때문에, 아래 두 보너스 기능은 이번 제출 범위에서 실제로 구현하지는 않았습니다. 다만 두 기능이 서로 자연스럽게 이어지는 아이디어라 설계 방향을 함께 정리해 둡니다.

> 여러 지역을 추천받아 비교하려면 API 호출 횟수가 늘어나는데, 결과 캐싱을 적용하면 이미 조회한 날짜·지역 조합은 재호출 없이 즉시 재사용할 수 있습니다. 즉 **캐싱은 복수 지역 추천의 확장성을 뒷받침하는 기반 기능**이 됩니다.

### 14-1. 복수 지역 추천 (Multiple City Recommendations)

- **목적**: 한 번의 실행으로 여러 후보 지역을 비교할 수 있도록 확장
- **설계 방향**
  - LLM 프롬프트를 "도시 1곳 추천"에서 "도시 N곳을 우선순위와 함께 추천"으로 변경하고, JSON 스키마를 배열 형태(`{"recommendations": [{"city": ..., "weather": ..., ...}, ...]}`)로 확장
  - 추천된 도시마다 `search_restaurants()`를 반복 호출해 도시별 맛집 목록을 확보
  - `generate_report()`에서 지역별 섹션(`## 추천 지역 1 - 제주도`, `## 추천 지역 2 - 강릉` ...)으로 리포트를 구성
  - CLI에 `--count N` 같은 선택적 옵션을 추가해 사용자가 추천받을 지역 수를 조절할 수 있게 설계
- **기대 효과**: 여행지를 비교해서 고르고 싶은 사용자에게 선택지를 제공, 단일 추천의 "정보 부족" 한계를 보완

### 14-2. 결과 캐싱 (간단한 파일 기반 캐시)
...
- **8장(캐시 파일 관리)과의 차이**: 8장에서 다룬 `__pycache__`/`.pyc`는 파이썬 인터프리터가 코드를 다시 컴파일하지 않도록 돕는 **실행 최적화 캐시**이고, 이번 14-2의 결과 캐싱은 **API 호출 결과(데이터)를 재사용하는 애플리케이션 레벨 캐시**입니다. 성격은 다르지만 "한 번 계산·조회한 값을 저장해 두었다가 재사용한다"는 개념은 동일합니다.

### 14-3. 구현 여부

두 기능 모두 설계 방향과 예시 코드만 정리했으며, 실제 코드에는 반영하지 않았습니다. 이후 확장한다면 위 스케치를 기반으로 `get_recommendation()`·`search_restaurants()`·`main()`을 수정하는 것으로 시작할 수 있습니다.
