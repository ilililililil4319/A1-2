...
- **8장(캐시 파일 관리)과의 차이**: 8장에서 다룬 `__pycache__`/`.pyc`는 파이썬 인터프리터가 코드를 다시 컴파일하지 않도록 돕는 **실행 최적화 캐시**이고, 이번 15-1의 결과 캐싱은 **API 호출 결과(데이터)를 재사용하는 애플리케이션 레벨 캐시**입니다. 성격은 다르지만 "한 번 계산·조회한 값을 저장해 두었다가 재사용한다"는 개념은 동일합니다.
