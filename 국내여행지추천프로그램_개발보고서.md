...
VS Code는 설치 직후 기본적으로 영어로 실행된다. 메뉴를 한글로 보기 위해 아래 순서로 한국어 언어팩을 설치했다.

- VS Code를 실행하고 왼쪽 사이드바에서 확장(Extensions) 아이콘 클릭 (또는 Ctrl+Shift+X)
- 검색창에 "Korean" 또는 "한국어" 입력
- "Korean Language Pack for Visual Studio Code" 확장을 찾아 Install 클릭
- 설치가 끝나면 화면 오른쪽 아래에 "언어를 변경하려면 다시 시작하세요"라는 안내 팝업이 뜨는데, 여기서 Change Language and Restart 버튼을 클릭
- VS Code가 자동으로 껐다 켜지면서 메뉴가 한글로 표시됨

팝업을 놓쳤거나 나타나지 않은 경우, Ctrl+Shift+P로 명령 팔레트를 열고 "Configure Display Language"를 입력해 ko를 선택한 뒤 VS Code를 직접 껐다 켜면 동일하게 적용된다.
...
*그림 33. GitHub 저장소(A1-2, main 브랜치, 37 Commits) — .env.example, .gitignore, 2025-03-15/2025-09-15/2025-10-01 데이터·리포트, README.md, requirements.txt, travel_planner.py, 국내여행지추천프로그램_개발보고서.md/.pdf까지 최신 상태로 업로드 완료*
...
| GitHub 업로드 | A1-2 저장소 main 브랜치에 13개 파일 최종 반영(보고서 포함) | V |
...
*그림 44. GitHub 저장소(A1-2) — travel_planner.py, README.md, requirements.txt, .env.example, .gitignore, 2025-03-15/2025-09-15/2025-10-01 결과 파일에 이어 국내여행지추천프로그램_개발보고서.md/.pdf까지 업로드 완료된 최종 모습(37 Commits)*
