...
README.md에는 미션 개요, 기능 목록, 프로그램 흐름도, 설치 방법, API 키 설정(세션 환경변수/.env 두 방식), 실행 방법과 출력 예시, 결과물(JSON/Markdown) 형식 설명, 에러 처리 정책 표, 실제 테스트 검증 내역, 자체 점검표, 공용PC 보안 주의사항, 트러블슈팅 Q&A, 전체 소스 코드까지 포함해 이 문서 하나만으로 프로젝트를 이해·재현할 수 있도록 구체적으로 작성했다.
...
*그림 46. GitHub — travel_planner.py 원본 (398 lines / 15.5KB) — 보너스 과제 반영 이전의 GitHub 업로드본. 보너스 과제(복수 지역 추천·결과 캐싱)가 반영된 최종 버전은 13장·18장 참고*
...
*그림 49. PowerShell — --count 2로 실행한 결과, 부산·경주 2개 지역이 한 번에 추천되고 [1/3]~[3/3] 전 과정이 정상 완료된 화면*

![](images/bonus3.png)

*그림 50. VS Code — 2025-10-01_travel_plan.md 결과. "추천 지역 1 – 부산", "추천 지역 2 – 경주"로 지역별 섹션이 분리되어 생성됨*

![](images/bonus4.png)

*그림 51. VS Code — 2025-10-01_data.json 결과. "regions" 배열 안에 지역별 recommendation·restaurants가 구조화되어 저장됨*

![](images/bonus5.png)

*그림 52. results 폴더 — 기존 2025-03-15·2025-09-15 결과는 그대로 보존된 채 2025-10-01 결과(2개 지역 포함, 5KB)만 새로 추가된 모습*

### 18-3. 결과 캐싱 — 실행 결과

![](images/bonus6.png)

*그림 53. PowerShell — --refresh 없이 동일한 --date로 재실행 시 [CACHE] 기존 결과를 재사용합니다 메시지와 함께 [1/3]·[2/3] 단계 없이 바로 [3/3]로 진행되는 화면*

![](images/bonus7.png)

*그림 54. PowerShell — --refresh를 붙여 같은 날짜로 재실행하면 캐시를 무시하고 [1/3]부터 다시 API를 호출해, 이번에는 경주 대신 전주가 추천됨(실제로 API가 재호출되었음을 증명)*

### 18-4. 최종 프로젝트 폴더

![](images/bonus1.png)

*그림 55. 최종 travel_project 폴더 — travel_planner.py가 보너스 기능이 반영된 19KB 버전으로 교체된 모습*
